...
export function Header() {
  ...
  const navigation = [
    ...
    { name: "تطبیق هوشمند", href: "/ai-matching" },
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3">
            ...
            <span className="font-bold text-[#02153D] font-sans text-sm">اپلای‌گرافی</span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-6">
            ...
            {navigation.slice(9).map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-sm font-medium text-gray-700 hover:text-[#FF6A5C] transition-colors font-serif"
              >
                {item.name}
              </Link>
            ))}
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 border-t">
              ...
              <div className="pt-4 space-y-2">
                <Button
                  asChild
                  variant="outline"
                  className="w-full border-[#FF6A5C] text-[#FF6A5C] hover:bg-[#FF6A5C] hover:text-white"
                >
                  <Link href="tel:+989330578976">تماس رایگان: ۰۹۳۳۰۵۷۸۹۷۶</Link>
                </Button>
